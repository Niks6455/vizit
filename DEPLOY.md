...
2. **У регистратора домена (где куплен keykap.space):** настрой DNS.
   - **Корень домена (keykap.space):** CNAME для корня запрещён стандартом DNS. Нужны **4 записи типа A** с хостом `@` (или «корневой домен»), каждая со своим значением:
     - `185.199.108.153`
     - `185.199.109.153`
     - `185.199.110.153`
     - `185.199.111.153`
   - **Поддомен www (по желанию):** одна запись **CNAME** с хостом `www`, значение — `niks6455.github.io`.
