...
const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  const getDocumentTop = useCallback((element: HTMLElement) => {
    // IMPORTANT: use offsetTop chain (stable even when elements are transformed).
    // getBoundingClientRect() includes transforms, which causes feedback/jitter
    // when we also set transform every frame.
    let top = 0;
  ...
  const getElementOffset = useCallback(
    (element: HTMLElement) => {
      if (useWindowScroll) {
        return getDocumentTop(element);
      } else {
        return element.offsetTop;
      }
    },
    [getDocumentTop, useWindowScroll],
  );
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    // PERF: blur "top card index" is computed once (not for every card).
    let topCardIndex = 0;
    if (blurAmount) {
    ...
    cardsRef.current.forEach((card, i) => {
      ...
      const newTransform = {
        // Use 0.5px steps to reduce subpixel shimmering.
